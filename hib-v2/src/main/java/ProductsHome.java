...
public class ProductsHome {
	...
	public Products merge(Products detachedInstance) {
		...
		try {
			Products result = (Products) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Products findById(java.lang.String id) {
		...
		try {
			Products instance = (Products) sessionFactory.getCurrentSession().get("Products", id);
			if (instance == null) {
	...
	public List findByExample(Products instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Products").add(Example.create(instance))
					.list();
