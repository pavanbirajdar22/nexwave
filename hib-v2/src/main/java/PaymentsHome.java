...
public class PaymentsHome {
	...
	public Payments merge(Payments detachedInstance) {
		...
		try {
			Payments result = (Payments) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Payments findById(PaymentsId id) {
		log.debug("getting Payments instance with id: " + id);
		try {
			Payments instance = (Payments) sessionFactory.getCurrentSession().get("Payments", id);
			if (instance == null) {
	...
	public List findByExample(Payments instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Payments").add(Example.create(instance))
					.list();
