...
public class CustomersHome {
	...
	public Customers findById(int id) {
		...
		try {
			Customers instance = (Customers) sessionFactory.getCurrentSession().get("Customers", id);
			if (instance == null) {
	...
	public List findByExample(Customers instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Customers").add(Example.create(instance))
					.list();
